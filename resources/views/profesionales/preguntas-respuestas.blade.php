            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="preguntas-tab" data-toggle="tab" href="#preguntas" role="tab"
                        aria-controls="preguntas" aria-selected="true">
                        <i class="fas fa-question-circle"></i> Preguntas sin responder
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="mis-respuestas-tab" data-toggle="tab" href="#mis-respuestas" role="tab"
                        aria-controls="mis-respuestas" aria-selected="false">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    @if ($pregunta->categoria)
                                                        <span
                                                            class="badge badge-success">{{ $pregunta->categoria->nombre }}</span>
                                                    @endif
                                                    @if ($pregunta->especialidad)
                                                        <span
                                                            class="badge badge-primary">{{ $pregunta->especialidad->nombre }}</span>
                                                    @endif
                                                    @if ($pregunta->subespecialidad)
                                                        <span
                                                            class="badge badge-secondary">{{ $pregunta->subespecialidad->nombre }}</span>
                                                    @endif
                                                </div>
                                                <small
                                                    class="text-muted">{{ $pregunta->created_at->format('d/m/Y H:i') }}</small>
                                            </div>
                                        </div>
                                        <div class="card-body">
                                            <h5 class="card-title mb-2"> <strong>Pregunta:</strong></h5>
                                            <p class="card-text">{{ $pregunta->pregunta }}</p>

                                            @php
                                                $yaRespondida =$pregunta->respuestas->where('profesional_id', $profesional->id)->count() > 0;
                                                $respuesta = $pregunta->respuestas->where('profesional_id', $profesional->id)->first();
                                            @endphp

                                            @if (!$yaRespondida)
                                                <button class="btn btn-success btn-sm"
                                                    onclick="mostrarFormularioRespuesta({{ $pregunta->id }})">
                                            ...
                                            @else
                                                ...
                                                <div class="alert alert-info">
                                                    ...
                                                    <small
                                                        class="text-muted">({{ date('d/m/Y H:i', strtotime($respuesta->created_at)) }})</small>
                                                    <p class="mb-0 mt-2">{{ $respuesta->respuesta }}</p>
                                                </div>
                                            @endif

                                            <!-- Formulario de respuesta (inicialmente oculto) -->
                                            <div id="form-respuesta-{{ $pregunta->id }}" class="mt-3"
                                                style="display: none;">
                                                <div class="form-group">
                                                    <label for="respuesta-{{ $pregunta->id }}">Tu respuesta:</label>
                                                    <textarea class="form-control" id="respuesta-{{ $pregunta->id }}" rows="4"
                                                        placeholder="Escribe tu respuesta aquí..."></textarea>
                                                </div>
                                                <button type="button" class="btn btn-primary btn-sm"
                                                    onclick="enviarRespuesta({{ $pregunta->id }})">
                                                    <i class="fas fa-paper-plane"></i> Enviar respuesta
                                                </button>
                                                <button type="button" class="btn btn-secondary btn-sm"
                                                    onclick="ocultarFormularioRespuesta({{ $pregunta->id }})">
                                                    Cancelar
                                                </button>
                                            </div>

                                            <!-- Mostrar otras respuestas si existen -->
                                            @if ($pregunta->respuestas->where('profesional_id' != $profesional->id)->count() > 0)
                                                <div class="mt-3">
                                                    <h6>Otras respuestas de profesionales:</h6>
                                                    @foreach ($pregunta->respuestas as $respuesta)
                                                        <div class="alert alert-info">
                                                            <strong>
                                                                {{ $respuesta->profesional->nombre_completo }}</strong>
                                                            <small
                                                                class="text-muted">({{ date('d/m/Y H:i', strtotime($respuesta->created_at)) }})</small>
                                                            <p class="mb-0 mt-2">{{ $respuesta->respuesta }}</p>
                                                        </div>
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    @if ($respuesta->pregunta->categoria)
                                                        <span
                                                            class="badge badge-success">{{ $respuesta->pregunta->categoria->nombre }}</span>
                                                    @endif
                                                    @if ($respuesta->pregunta->especialidad)
                                                        <span
                                                            class="badge badge-primary">{{ $respuesta->pregunta->especialidad->nombre }}</span>
                                                    @endif
                                                    @if ($respuesta->pregunta->subespecialidad)
                                                        <span
                                                            class="badge badge-secondary">{{ $respuesta->pregunta->subespecialidad->nombre }}</span>
                                                    @endif
                                                </div>
                                                <small class="text-muted">Respondida el
                                                    {{ date('d/m/Y H:i', strtotime($respuesta->created_at)) }})</small></small>
                                            </div>
                                        </div>
                                        <div class="card-body">
                                            <h5 class="card-title mb-2"> <strong>Pregunta:</strong></h5>
