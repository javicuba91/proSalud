...
@section('title', 'Editar Etiqueta del Blog')

@section('content_header')
    <h1>Editar Etiqueta del Blog</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('blog.etiquetas.update', $etiqueta) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <input type="color" class="form-control @error('color') is-invalid @enderror"
                           id="color" name="color" value="{{ old('color', $etiqueta->color) }}" required>
                    @error('color')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
