...
@section('content')
    <div class="card">
        <div class="card-body">
            <h5>Proveedor: {{ $suscripcion->proveedor->nombre ?? 'N/A' }}</h5>
            <h5>Plan: {{ $suscripcion->plan->nombre ?? 'N/A' }}</h5>
            <h5>Monto: ${{ $suscripcion->plan->precio ?? '0.00' }}</h5>
            <form action="{{ route('admin.facturacion.proveedor.pagar.post', $suscripcion->id) }}" method="POST">
                @csrf
                ...
                <a href="{{ route('admin.facturacion.proveedor.index') }}" class="btn btn-secondary">Cancelar</a>
            </form>
        </div>
    </div>
@endsection
